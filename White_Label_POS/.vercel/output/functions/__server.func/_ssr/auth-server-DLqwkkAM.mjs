...
var loginServerFn = createServerFn().validator((d) => d).handler(loginServerFn_createServerFn_handler, async ({ data }) => {
	...
	try {
		...
		if (tenantStatus === "Archived") throw new Error("Tenant is archived and cannot be accessed");
		return {
			success: true,
			user: {
				id,
	...
	try {
		...
		const [cashier, till, scheduledShift, activeTillShift] = await Promise.all([
			db.query.staffUsers.findFirst({
				where: and(eq(staffUsers.id, cashierId), eq(staffUsers.tenantId, tenantId), eq(staffUsers.branchId, branchId), eq(staffUsers.role, "cashier"), eq(staffUsers.isActive, true)),
				with: { tenant: true }
			}),
			db.query.tills.findFirst({ where: and(eq(tills.id, tillId), eq(tills.tenantId, tenantId), eq(tills.branchId, branchId)) }),
			db.query.shifts.findFirst({ where: and(eq(shifts.cashierId, cashierId), eq(shifts.shiftDate, today), eq(shifts.status, "Scheduled")) }),
			db.query.shifts.findFirst({ where: and(eq(shifts.tillId, tillId), eq(shifts.status, "Open")) })
		]);
		if (!cashier) {
		...
		if (cashier.tenant?.status === "Archived") throw new Error("Tenant is archived and cannot be accessed");
		return {
			success: true,
			user: {
				id: cashier.id,

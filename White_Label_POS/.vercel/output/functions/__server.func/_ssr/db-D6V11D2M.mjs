import { n as __exportAll, t as TSS_SERVER_FUNCTION } from "./server-BlyqvE9x.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/db-D6V11D2M.js
...
var schema_exports = /* @__PURE__ */ __exportAll({
	...
	aggregatorSyncSettings: () => aggregatorSyncSettings,
	auditLogs: () => auditLogs,
	batches: () => batches,
	...
	customerTransactionsRelations: () => customerTransactionsRelations,
	customers: () => customers,
	customersRelations: () => customersRelations,
	grn: () => grn,
	grnItems: () => grnItems,
	grnItemsRelations: () => grnItemsRelations,
	grnRelations: () => grnRelations,
	...
	orderPaymentsRelations: () => orderPaymentsRelations,
	orders: () => orders,
	ordersRelations: () => ordersRelations,
	platformSettings: () => platformSettings,
	priceOverrideRequests: () => priceOverrideRequests,
	priceOverrideRequestsRelations: () => priceOverrideRequestsRelations,
	...
	staffUsersRelations: () => staffUsersRelations,
	stockAdjustments: () => stockAdjustments,
	stockLevels: () => stockLevels,
	stockLevelsRelations: () => stockLevelsRelations,
	stockTransfers: () => stockTransfers,
	tenantSettings: () => tenantSettings,
	tenantSettingsRelations: () => tenantSettingsRelations,
	tenants: () => tenants,
	tenantsRelations: () => tenantsRelations,
	tills: () => tills,
	tillsRelations: () => tillsRelations,
	unitConversions: () => unitConversions,
	vendorInvoices: () => vendorInvoices,
	vendorInvoicesRelations: () => vendorInvoicesRelations,
...
var tenants = pgTable("tenants", {
	...
	monthlyOrderLimit: integer("monthly_order_limit").notNull().default(1e4),
	createdAt: timestamp("created_at").defaultNow().notNull()
});
var tenantSettings = pgTable("tenant_settings", {
	id: uuid("id").primaryKey().defaultRandom(),
	...
	vatInclusive: boolean("vat_inclusive").notNull().default(true),
	loyaltyRedemptionRate: decimal("loyalty_redemption_rate", {
		precision: 5,
		scale: 2
	}).notNull().default("0.01"),
	loyaltyPointsPerAed: integer("loyalty_points_per_aed").notNull().default(10),
	loyaltyMinPointsToRedeem: integer("loyalty_min_points_to_redeem").notNull().default(5e3),
	currency: text("currency").notNull().default("AED"),
...
var products = pgTable("products", {
	...
	return { skuIdx: uniqueIndex("products_tenant_sku_idx").on(table.tenantId, table.sku) };
});
var productBarcodes = pgTable("product_barcodes", {
	id: uuid("id").primaryKey().defaultRandom(),
...
var unitConversions = pgTable("unit_conversions", {
	id: uuid("id").primaryKey().defaultRandom(),
	productId: uuid("product_id").notNull().references(() => products.id, { onDelete: "cascade" }),
...
var stockLevels = pgTable("stock_levels", {
	id: uuid("id").primaryKey().defaultRandom(),
	productId: uuid("product_id").notNull().references(() => products.id, { onDelete: "cascade" }),
...
}, (table) => ({ branchIdx: index("stock_levels_branch_idx").on(table.branchId) }));
var stockTransfers = pgTable("stock_transfers", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	productId: uuid("product_id").notNull().references(() => products.id, { onDelete: "cascade" }),
	sourceBranchId: uuid("source_branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
	destinationBranchId: uuid("destination_branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
	quantity: integer("quantity").notNull(),
	transferredBy: uuid("transferred_by").references(() => staffUsers.id),
	status: text("status").notNull().default("Completed"),
	createdAt: timestamp("created_at").defaultNow().notNull()
});
var stockAdjustments = pgTable("stock_adjustments", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	branchId: uuid("branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
	productId: uuid("product_id").notNull().references(() => products.id, { onDelete: "cascade" }),
...
var inventoryLedger = pgTable("inventory_ledger", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	branchId: uuid("branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
...
var priceOverrideRequests = pgTable("price_override_requests", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	branchId: uuid("branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
	productId: uuid("product_id").notNull().references(() => products.id, { onDelete: "cascade" }),
	...
	requestedPrice: decimal("requested_price", {
		precision: 10,
		scale: 2
	}).notNull(),
	reason: text("reason").notNull(),
...
var promotionBranches = pgTable("promotion_branches", {
	id: uuid("id").primaryKey().defaultRandom(),
...
var grnItems = pgTable("grn_items", {
	id: uuid("id").primaryKey().defaultRandom(),
...
var vendorInvoices = pgTable("vendor_invoices", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	vendorId: uuid("vendor_id").notNull().references(() => vendors.id),
...
var customers = pgTable("customers", {
	...
	storeCredit: decimal("store_credit", {
		precision: 12,
		scale: 2
	}).notNull().default("0.00"),
...
var customerTransactions = pgTable("customer_transactions", {
	id: uuid("id").primaryKey().defaultRandom(),
...
var shifts = pgTable("shifts", {
	...
	cashDrops: text("cash_drops").default("[]"),
...
var invoiceSequences = pgTable("invoice_sequences", {
	tenantId: uuid("tenant_id").primaryKey().references(() => tenants.id, { onDelete: "cascade" }),
	currentValue: integer("current_value").notNull().default(0)
});
var orderPayments = pgTable("order_payments", {
	id: uuid("id").primaryKey().defaultRandom(),
...
var aggregatorCredentials = pgTable("aggregator_credentials", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	branchId: uuid("branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
...
var aggregatorOrders = pgTable("aggregator_orders", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	branchId: uuid("branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
...
var aggregatorSyncSettings = pgTable("aggregator_sync_settings", {
	id: uuid("id").primaryKey().defaultRandom(),
	tenantId: uuid("tenant_id").notNull().references(() => tenants.id, { onDelete: "cascade" }),
	branchId: uuid("branch_id").notNull().references(() => branches.id, { onDelete: "cascade" }),
	platform: text("platform").notNull(),
...
var staffUsersRelations = relations(staffUsers, ({ one, many }) => ({
	tenant: one(tenants, {
...
var batchesRelations = relations(batches, ({ one }) => ({
	tenant: one(tenants, {
...
var ordersRelations = relations(orders, ({ one, many }) => ({
	...
	cashier: one(staffUsers, {
		fields: [orders.cashierId],
		references: [staffUsers.id]
	}),
	till: one(tills, {
		fields: [orders.tillId],
		references: [tills.id]
	}),
...
var customerTransactionsRelations = relations(customerTransactions, ({ one }) => ({
	customer: one(customers, {
		fields: [customerTransactions.customerId],
		references: [customers.id]
	}),
	tenant: one(tenants, {
		fields: [customerTransactions.tenantId],
		references: [tenants.id]
	})
}));
...
var grnRelations = relations(grn, ({ one, many }) => ({
	tenant: one(tenants, {
		fields: [grn.tenantId],
		references: [tenants.id]
	}),
...
var grnItemsRelations = relations(grnItems, ({ one }) => ({
	grn: one(grn, {
...
var vendorInvoicesRelations = relations(vendorInvoices, ({ one }) => ({
	tenant: one(tenants, {
		fields: [vendorInvoices.tenantId],
		references: [tenants.id]
	}),
	vendor: one(vendors, {
	...
	grn: one(grn, {
		fields: [vendorInvoices.grnId],
		references: [grn.id]
	})
}));
var tenantSettingsRelations = relations(tenantSettings, ({ one }) => ({ tenant: one(tenants, {
	fields: [tenantSettings.tenantId],
	references: [tenants.id]
}) }));
var promotionsRelations = relations(promotions, ({ one }) => ({ tenant: one(tenants, {
...
var shiftsRelations = relations(shifts, ({ one }) => ({
	...
	till: one(tills, {
		fields: [shifts.tillId],
		references: [tills.id]
	})
}));
var orderPaymentsRelations = relations(orderPayments, ({ one }) => ({ order: one(orders, {
...
var rolePermissionsRelations = relations(rolePermissions, ({ one }) => ({ tenant: one(tenants, {
	fields: [rolePermissions.tenantId],
	references: [tenants.id]
}) }));
var priceOverrideRequestsRelations = relations(priceOverrideRequests, ({ one }) => ({
	tenant: one(tenants, {
		fields: [priceOverrideRequests.tenantId],
		references: [tenants.id]
	}),
	branch: one(branches, {
		fields: [priceOverrideRequests.branchId],
		references: [branches.id]
	}),
	product: one(products, {
		fields: [priceOverrideRequests.productId],
		references: [products.id]
	}),
...
var tillsRelations = relations(tills, ({ one }) => ({
	tenant: one(tenants, {
		fields: [tills.tenantId],
		references: [tenants.id]
	}),
	branch: one(branches, {
		fields: [tills.branchId],
		references: [branches.id]
	}),
	creator: one(staffUsers, {
		fields: [tills.createdBy],
		references: [staffUsers.id]
	})
}));
...
//#endregion
export { tenantSettings as A, rolePermissions as C, stockAdjustments as D, staffUsers as E, vendorPayments as F, vendors as I, createServerRpc as L, tills as M, unitConversions as N, stockLevels as O, vendorInvoices as P, purchaseOrders as S, shifts as T, productBarcodes as _, customerTransactions as a, promotions as b, grnItems as c, loginAttempts as d, orderItems as f, priceOverrideRequests as g, platformSettings as h, branches as i, tenants as j, stockTransfers as k, inventoryLedger as l, orders as m, auditLogs as n, customers as o, orderPayments as p, batches as r, grn as s, db as t, invoiceSequences as u, productVariants as v, schema_exports as w, purchaseOrderItems as x, products as y };

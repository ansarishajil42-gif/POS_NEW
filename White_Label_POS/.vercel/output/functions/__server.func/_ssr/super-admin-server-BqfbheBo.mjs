...
import { A as tenantSettings, E as staffUsers, L as createServerRpc, h as platformSettings, i as branches, j as tenants, m as orders, n as auditLogs, t as db } from "./db-z-IRNgLG.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/super-admin-server-BqfbheBo.js
...
var updateTenantLimitsServerFn = createServerFn({ method: "POST" }).validator((d) => d).handler(updateTenantLimitsServerFn_createServerFn_handler, async ({ data }) => {
	...
	try {
		return await db.transaction(async (tx) => {
			...
			const { tills } = await import("./db-z-IRNgLG.mjs").then((n) => n.w);

...
async function main() {
  ...
  // 4. Users
  const cashier1Pin = await bcrypt.hash("1234", 10);
  const cashier2Pin = await bcrypt.hash("5678", 10);

  await db.insert(schema.staffUsers).values([
    {
      email: "superadmin@cloudynationpos.com",
      passwordHash: await bcrypt.hash("superadmin@cloudynationpos.com", 10),
      role: "super_admin",
    },
    {
      email: "admin@almadina.com",
      passwordHash: await bcrypt.hash("admin@almadina.com", 10),
      role: "head_office_admin",
      tenantId: tenantMadina.id,
    },
    {
      email: "manager@almadina.com",
      passwordHash: await bcrypt.hash("manager@almadina.com", 10),
      role: "branch_manager",
      tenantId: tenantMadina.id,
    ...
    },
    {
      ...
      passwordHash: await bcrypt.hash("inventory@almadina.com", 10),
    ...
    },
    {
      ...
      passwordHash: await bcrypt.hash("purchasing@almadina.com", 10),
      role: "purchasing_officer",
      tenantId: tenantMadina.id,
    },
    {
      email: "cashier@almadina.com",
      passwordHash: await bcrypt.hash("cashier@almadina.com", 10),
      pinHash: cashier1Pin,
      role: "cashier",
      tenantId: tenantMadina.id,
      branchId: madinaBarsha.id,
  ...
  const vendor1 = (await db.insert(schema.vendors).values({
    ...
    passwordHash: await bcrypt.hash("vendor@globalfmcg.ae", 10),

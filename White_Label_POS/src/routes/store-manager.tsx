...
function StoreManager() {
  ...
  try {
    ...
    const todayOrders = data.orders.filter((o: any) => new Date(o.createdAt).toISOString().split("T")[0] === today);
    const salesToday = todayOrders.reduce((sum: number, o: any) => sum + (Number(o.total) || 0), 0) || 0;
    ...
    todayOrders.forEach((o: any) => {
      (o.items || []).forEach((i: any) => {
        if (!itemCounts[i.productId]) itemCounts[i.productId] = { name: i.product?.name || "Unknown", qty: 0, value: 0 };
        itemCounts[i.productId].qty += (Number(i.qty) || 0);
        itemCounts[i.productId].value += (Number(i.unitPrice) || 0) * (Number(i.qty) || 0);
      });
    });
    const topItems = Object.values(itemCounts).sort((a: any, b: any) => b.qty - a.qty).slice(0, 4);
    ...
    const yesterdayOrders = data.orders.filter((o: any) => new Date(o.createdAt).toDateString() === yesterday.toDateString());
    const salesYesterday = yesterdayOrders.reduce((sum: number, o: any) => sum + (Number(o.total) || 0), 0);
    const salesGrowth = salesYesterday > 0 ? ((salesToday - salesYesterday) / salesYesterday * 100).toFixed(1) : 0;
    const growthText = salesYesterday > 0 ? `${Number(salesGrowth) >= 0 ? '+' : ''}${salesGrowth}% vs yesterday` : undefined;
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayOrders = data.orders.filter((o: any) => new Date(o.createdAt).toDateString() === date.toDateString());
          ...
          <main className="min-w-0 flex-1">
            <TabsContent value="dashboard" className="mt-0 space-y-5">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard label="Sales today" value={aedShort(salesToday)} delta={growthText} icon={TrendingUp} tone="success" />
                <StatCard label="Transactions" value={transactions.toLocaleString()} delta={`Avg basket: ${avgBasket}`} icon={Receipt} />
                <StatCard label="Low stock items" value={lowStock} icon={AlertTriangle} tone="accent" />
                <StatCard label="Active tills" value={`${activeTills} / ${totalTills}`} icon={Monitor} />
              </div>
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div className="panel p-6">
                  ...
                  <div className="mt-4 space-y-4">
                    {topItems.length === 0 && <p className="text-sm text-muted-foreground">No sales yet today.</p>}
              ...
              <div className="panel overflow-hidden p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="relative w-72">
                    ...
                    <Input placeholder="Scan barcode or search SKU..." className="pl-9 h-10 rounded-xl text-sm bg-surface-2 border-transparent focus:border-primary" />
                  </div>
                  <Button variant="outline" className="h-10 rounded-xl">Filter by Category</Button>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl transition-colors duration-300 ${isLow ? 'bg-accent/10 text-accent group-hover:bg-accent/20' : 'bg-primary/10 text-primary group-hover:bg-primary/20'}`}>
                            <Package className="h-6 w-6" />
                          </div>
                          <div>
                            <h4 className="text-base font-bold text-ink transition-colors group-hover:text-primary">{p.productName}</h4>
                        ...
                        <div className="flex items-center gap-8 text-right">
                          <div>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">In Stock</p>
                            <p className={`text-lg font-extrabold ${isLow ? 'text-accent' : 'text-ink'}`}>
                              {localQty} <span className="text-sm font-semibold text-muted-foreground">{p.unit}</span>
                            </p>
                          </div>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {data.stock.filter((s: any) => s.priceOverride).length === 0 && (
                      <tr><td colSpan={4} className="px-4 py-4 text-center text-muted-foreground">No active price overrides.</td></tr>
                    )}
                    {data.stock.filter((s: any) => s.priceOverride).map((s: any) => (
                      <tr key={s.id} className="hover:bg-surface-2/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-muted-foreground">{aed(Number(s.basePrice))}</td>
                        <td className="px-4 py-3 font-bold text-ink">{aed(Number(s.priceOverride))}</td>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {data.shifts.length === 0 && (
                      <tr><td colSpan={4} className="px-4 py-4 text-center text-muted-foreground">No shifts found for today.</td></tr>
                    )}
                    {data.shifts.map((shift: any) => {
                      ...
                      return (
                        <tr key={shift.id} className="hover:bg-surface-2/50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-muted-foreground">{shift.tillId || "-"}</td>
                          <td className="px-4 py-3 text-muted-foreground">
                             {new Date(shift.openedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {isCompleted ? new Date(shift.closedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Ongoing'}
                          </td>
                          <td className="px-4 py-3 text-right">
                            {!isCompleted && shift.status === "Active" && <span className="inline-flex items-center gap-1.5 rounded-full bg-success/10 px-2.5 py-1 text-xs font-bold text-success"><span className="h-1.5 w-1.5 rounded-full bg-success"/> Active</span>}
                            {isCompleted && <span className="inline-flex items-center gap-1.5 rounded-full bg-border/50 px-2.5 py-1 text-xs font-bold text-muted-foreground">Completed</span>}
                          </td>
                        </tr>
                      );
                    })}
